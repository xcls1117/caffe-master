...
namespace caffe {
	...
	void MTCNNDataLayer<Dtype>::Forward_cpu(
		...
		Batch<Dtype>* batch = this->prefetch_full_.pop("Data layer prefetch queue empty"); //ly,
		...
		this->prefetch_free_.push(batch);//ly,
	}
	...
	void MTCNNDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
		...
		Dtype* top_pts = this->output_pts_ ? batch->pts_.mutable_cpu_data() : 0; //ly,
