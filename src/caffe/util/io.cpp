...
namespace caffe {
	...
	bool ReadProtoFromBinaryFile(const char* filename, Message* proto) {
		int fd = open(filename, O_RDONLY ); //ly,
		CHECK_NE(fd, -1) << "File not found: " << filename;
	...
	bool ReadImageToMTCNNDatum(const string& filename, const vector<float>& label,
		...
		//bool succ = ReadImageToDatum(filename, { label[0] }, height, width, is_color, encoding, datum->mutable_datum()); //ly,
		bool succ = ReadImageToDatum(filename, label, height, width, is_color, encoding, datum->mutable_datum());
		...
		if (succ){
			...
			::google::protobuf::RepeatedField< float > *pts = datum->mutable_pts();
			pts->Clear();
